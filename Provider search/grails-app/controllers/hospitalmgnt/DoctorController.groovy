...
class DoctorController {
	...
	def displayimage() {
			...
			def doctor = Doctor.findByDoctorId(params.doctorId)
		byte[] imageInByte = doctor.image
	...
	def savedoctor() {
		...
		log.info(certifications+"KKKKKKKKKKKKKKKKKKKk")
		...
		if( ! (isValid(regId)&& isValid(dFirstname) && isValid(dLastname) && isValid(dAge)&& isValid(dGender)&& isValid(mobileNumber)
						...
						&& isValid(lattitude)&& isValid(longitude)&& isValid(certifications)&& isValid(choice) && isValid(myaction) && isValid(mode)&& isValid(expiredDate))){
					redirect(uri: "/user/doctorDashboard")
			return
		}
		if(myaction.equals("save")) {
			if( ! (isValid(regId)&& isValid(dFirstname) && isValid(dLastname) && isValid(dAge)&& isValid(dGender)&& isValid(mobileNumber)&& isValid(alternateMobile)
						...
						&& isValid(modifiedBy)&& isValid(fee)&& isValid(blood_flag)&& isValid(v_flag)&& isValid(qualifcation)
						&& isValid(lattitude)&& isValid(longitude)&& isValid(certifications)&& isValid(choice)&& isValid(expiredDate))){
				...
				def res=DoctorService.save(regId,mode,packName,status,dFirstname,dLastname,dAge,dGender,mobileNumber,alternateMobile,address,locality,
				...
				log.info(res)
				responseData.put(getMessages('default.status.label'),res.get("status"))
				responseData.put(getMessages('default.message.label'),res.get("message"))
				
			
				if(res.get("status") == "success"&& mode=="web"){
				...
				}else if(res.get("status") == "error" && mode=="web"){
	...
	def displayImage() {
		def doctorInstance = Doctor.findByDoctorId(params.doctorId)
		response.setContentType("image/png")
		response.outputStream << doctorInstance.image
		response.outputStream.flush()
	}
