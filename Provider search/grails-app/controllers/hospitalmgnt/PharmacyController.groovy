...
class PharmacyController  {
	...
	def displayimage() {
			...
			def pharmacy = Pharmacy.findByEmail(params.email)
        byte[] imageInByte = pharmacy.image
	...
	public HashMap drug_getdata1(drugName,locality) throws Exception{
		log.info("pharmacycontroller drug_getdata1 method")
		log.info(drugName)
		log.info(locality)
		...
		if((drugName==null || drugName=="") && (locality!=null || locality!="") ){
		 query.setQuery("locality:"+locality);
			...
			query.setQuery("status:1 && drug_name:"+drugName);
		   }
		else if((locality!=null || locality!="")&& (drugName!=null || drugName!="")){
			query.setQuery("status:1 && locality:"+locality+"&& drug_name:"+drugName);
		}
	...
	*/
	...
	def savepharmacy() {
		...
		if( ! (isValid(regId)&& isValid(firstname) && isValid(lastname) && isValid(mobileNumber) && isValid(address)&& isValid(locality)&& isValid(city)&& isValid(pin)&& isValid(email)&& 
						isValid(modifiedBy) && isValid(image) && isValid(myaction) && isValid(mode))){
					redirect(uri: "/user/pharmacyDashboard")
			return
		}
		if(myaction.equals("save")) {
			if( ! (isValid(regId)&& isValid(firstname) && isValid(lastname) && isValid(mobileNumber)&& isValid(alternateNumber) && isValid(address)&& isValid(locality)&& isValid(city)&& isValid(pin)&& isValid(email)&& isValid(password)
						&& isValid(pharmacyName) && isValid(image)&& isValid(modifiedBy))){
				...
				def res=pharmacyService.save(regId,packName,status,firstname,lastname,mobileNumber,alternateNumber,address,locality,
	...
	/* To get the messages */
		...
		protected void notFound() {
			request.withFormat {
				form multipartForm {
					flash.message = message(code: 'default.not.found.message', args: [message(code: 'Pharamcy.label', default: 'Image'), params.id])
					redirect action: "displayimage", method: "GET"
				}
