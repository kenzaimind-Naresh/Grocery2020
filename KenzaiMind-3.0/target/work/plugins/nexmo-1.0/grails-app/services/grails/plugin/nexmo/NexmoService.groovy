...
class NexmoService {
  ...
  def sendSms(String to, String text, String from=config?.sms?.default_from) throws NexmoException {
	  ...
	  log.info("nexmo service         "+to)
	  log.info("nexmo service         "+text)
	  log.info("nexmo service         "+from)
    ...
    def requestBody = [to: to, text: text, from: from, api_key: key, api_secret: secket]
	log.info("request")
