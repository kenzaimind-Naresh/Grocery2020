...
class AddressService {
	...
	def grailsApplication
		 ...
		 * save service for savePatient action in PatientController */
			def save(fullName,mobileNumber,pincode,houseNumber,area,landmark,city,state,deliverhere,userNameId,ugroceryNameId,ugroceryName,uquantity,ucost,uweight,uoffer,utotal,uimage,uname,utype){
			...
			try{
				...
				if(!check){
					...
					if(addressInstance){
						...
						log.info(userNameId)
						addressInstance.ugroceryNameId=ugroceryNameId
						log.info(ugroceryNameId)
						addressInstance.ugroceryName=ugroceryName
						log.info(ugroceryName)
						addressInstance.uquantity=uquantity
						log.info(uquantity)
						addressInstance.ucost=ucost
						log.info(ucost)
						addressInstance.uweight=uweight
						log.info(uweight)
						addressInstance.uoffer=uoffer
						log.info(uoffer)
						addressInstance.utotal=utotal
						log.info(utotal)

						addressInstance.uimage=uimage
						log.info(uimage)

						addressInstance.uname=uname
						log.info(uname)
						addressInstance.utype=utype
						log.info(utype)
