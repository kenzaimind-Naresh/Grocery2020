...
<div>
   ...
   <td style="color: blue;"><g:link controller="grocery" action="updateGrocery" id="${it.id}">${it.id}</g:link></td>
     <td style="color: blue;"><g:link controller="grocery" action="deleteGrocery" id="${it.id}">${it.id}</g:link></td>
           
<%--           	<g:form url="[resource:groceryInstance, action:'delete']" method="DELETE">--%>
<%--			--%>
...
<%--	--%>
    ...
    <g:if test="${result.offset/5 < (result.totalcount)/5-1}">
     <g:link type="button" style="color:white" controller="grocery" action="offsetlist"   params="${[offset: result.offset+5,mode:'web']}" value="Next"><button class="btn btn-primary">Next</button></g:link>
