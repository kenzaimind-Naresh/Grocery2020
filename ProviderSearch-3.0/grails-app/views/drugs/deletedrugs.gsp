...
<script type="text/javascript">
          $(document).ready(function(){
	          ...
	          drugName.blur(validateDrugName);
	          ...
	          drugName.keyup(validateDrugName);
	    ...
	    form.submit(function(){
	       if(validateDrugId() & validateDrugName() & validateRate() & validateModifiedBy()){
	       return true;
	       }else
	       {
	       return false;
	...
	function validateDrugName(){
        if(drugName.val().length < 3){
        	drugName.addClass("error");
        	drugNameinfo.text("Enter correct DrugName");
        	drugNameinfo.addClass("error");
        return false;
        }else
        {
        	drugName.removeClass("error");
        	drugNameinfo.text("please enter DrugName").hide();
        	drugNameinfo.addClass("error");
        return true;
        }
        }
   ...
   <div>
         <g:form controller="drugs" align="center" ID="myform" action="savedrugs"  nonvalidate="nonvalidate"> 
           ...
           <div class="fieldgroup">
            <tab4><label for="drugName">DrugName: </label></tab4>
            <tab5><input type="text" id="drugName" name="drugName"></tab5><br>
            <tab6><span id=drugNameinfo>Please enter DrugName</span></tab6><br>
            </div>
