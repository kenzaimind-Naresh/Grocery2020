...
class PackageController {
	...
	def purchasedpacklist(){
		
		log.info("PackageController purchasedpacklist Action")
		def responseData = new HashMap<>();
		def user= User.findByUsername(session.user)
		def username= session.user
		if(username ==null || username=="" ){
		...
		def mode="web"
		def of=0;
		def ppackdata=Doctor.list(sort:"id",order:"desc",max: 5, offset: of)
		log.info(ppackdata)
		def totalcount=Doctor.findAll().size()
		log.info(totalcount)
		responseData.put("listId", "ppacklist")
		responseData.put("totalcount",totalcount )
		responseData.put("ppackdata", ppackdata)
		responseData.put("user", user)
		responseData.put("offset", of)
		[result:responseData]
