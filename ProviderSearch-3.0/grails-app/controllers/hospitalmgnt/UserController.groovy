...
class UserController {
	...
	public HashMap provider_autofill() throws Exception{
		...
		HttpSolrServer solr=null;
		solr = new HttpSolrServer("http://localhost:8983/solr/ProviderSearch");
		...
		log.info("******************ddddd")
		HttpSolrServer solrobj=null;
		solrobj = new HttpSolrServer("http://localhost:8983/solr/ProviderSearch");
	...
	def pharmacySearchinfo(){
		
		log.info("UserController pharmacyinfo Action")
		def mode=params.mode
		def url="/user/pharmacyinfo.gsp"
		def city= params.city
		log.info(city)
		...
		def responseData = new HashMap<>()
		def msg;
		def solrresult = pharmacySearch_autofill();
		log.info("****************")
		def solrcity = provider_getdata2(city,pharmacyName);
		log.info(solrcity)
		if([city=="" || city==null] && [pharmacyName=="" || pharmacyName==null]){
			msg="Pharmacy you have Searched is not available"
		}
		else{
			msg=""
		}
		...
		responseData.put("solrresult", solrresult)
		responseData.put("city", city)
		responseData.put("pharmacyName", pharmacyName)
		responseData.put("message", msg)
	...
	public HashMap pharmacySearch_autofill() throws Exception{
		log.info("UserController pharmacySearch_autofill method")
		def city = params.city
		def pharmacyName = params.pharmacyName
		def dGender = params.dGender
		HttpSolrServer solr=null;
	...
	try{
			...
			for (int i = 0; i < results.size(); i++) {
				//solrdata.add(results.get(gi))
				providerdata.add(results.get(i).pharmacyName)
			}
			 Object[] st = solrdata.toArray();
	  for (Object s : st) {
		if (solrdata.indexOf(s) != solrdata.lastIndexOf(s)) {
	...
	public HashMap provider_getdata2(city,pharmacyName) throws Exception{
		log.info("UserController provider_getdata method")
		log.info(city)
		log.info(pharmacyName)
		...
		log.info(solrobj)
		 SolrQuery query = new SolrQuery();
		 if((pharmacyName==null || pharmacyName=="") && (city!=null || city!="") ){
			 query.setQuery("city:"+city);
			}
			else if((city==null || city=="") && (pharmacyName!=null || pharmacyName!="")  ){
				query.setQuery("pharmacyName:"+pharmacyName);
			   }
			else if((city!=null || city!="")&& (pharmacyName!=null || pharmacyName!="")){
				query.setQuery("city:"+city+"&& pharmacyName:"+pharmacyName);
			}
	...
	def pharmacyDashboard ={
		...
		log.info("UserController pharmacyDashboard Action")
		def mode=params.mode
		def url="/user/providerSearch.gsp"
		log.info("****************")
		def responseData = new HashMap<>()
	...
	//    def doctor=Doctor.getAll()
		...
		def user
		ArrayList<Doctor> cObject
		if(b){
		
		cObject=Doctor.findAllByEmailAndPassword(params.email,params.password);
		//user = Doctor.findByEmailAndPassword(params.email,params.password)
		log.info("Doctor with email in if "+cObject)
		}else{
		cObject = Doctor.findAllByMobileNumberAndPassword(params.email,params.password)
		log.info("Doctor with mobile number in else "+cObject)
		}
...
/*        if(next.after(current)){
		...
		if(user){
			...
			if(mode=="mobile"){
				...
				responseData.put("doctor", Doctor.findByEmail(cObject.email))
				render responseData as JSON
				return
	...
	def pharmacyvalidation = {
		...
		def user
		if(b){
		user = Pharmacy.findByEmailAndPassword(params.email,params.password)
		log.info("Doctor with email in if "+user)
		}else{
		user = Pharmacy.findByMobileNumberAndPassword(params.email,params.password)
		log.info("Doctor with mobile number in else "+user)
		}
