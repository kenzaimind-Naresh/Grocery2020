...
class DoctorService {
	...
	def UserService
		...
		def save(regId,packName,status,dFirstname,dLastname,dAge,dGender,mobileNumber,address,locality,
			...
			log.info(dataSource_solr)
					try{
						def check=Doctor.findByEmailOrMobileNumber(email,mobileNumber)
		...
		def update(regId,packName,status,dFirstname,dLastname,dAge,dGender,mobileNumber,address,locality,
			...
			String []args=["Doctor"]
			try{
				def doctorInstance=Doctor.findByEmailOrMobileNumber(email,mobileNumber)
				if(doctorInstance) {
					doctorInstance.regId=regId
					doctorInstance.packName=packName
					log.info(packName)
